...
@Component({
  selector: 'app-pdf-gas',
  templateUrl: './pdf-gas.page.html',
  styleUrls: ['./pdf-gas.page.scss'],
})
export class PdfGasPage implements OnInit {
...
// }
  ...
  // async loadResults(startDate, endDate) {
  ...
  // //   ref.where('fechaGas', '>=', fecha1) && ref.where('endDate', '<=', fecha2));
  ...
  async loadResults() {
    ...
    this.GasCollection = this.db.collection('ventasGas', ref => ref.orderBy('fechaGas', 'desc'));
    this.ventasGas = this.GasCollection.valueChanges();
    this.data = [];
    const userDoc = this.GasCollection;
    userDoc.get().subscribe((querySnapshot) => {
  ...
  async loadResultsLimit() {
    ...
    await loading.present();
    this.GasCollection = this.db.collection('ventasGas', ref => ref.orderBy('fechaGas', 'desc').limit(10));
    this.ventasGas = this.GasCollection.valueChanges();
    this.data = [];
    const userDoc = this.GasCollection;
    userDoc.get().subscribe((querySnapshot) => {
  ...
  async loadResultsLimitPequenos() {
    ...
    this.GasCollection = this.db.collection('ventasGas', ref => ref.where('tamanoGas', '==' , 'Pequeños').orderBy('fechaGas', 'desc').limit(10));
    this.ventasGas = this.GasCollection.valueChanges();
    this.data = [];
    const userDoc = this.GasCollection;
    userDoc.get().subscribe((querySnapshot) => {
  ...
  async loadResultsLimitMedianos() {
    ...
    // tslint:disable-next-line: triple-equals
    // tslint:disable-next-line: max-line-length
    this.GasCollection = this.db.collection('ventasGas', ref => ref.where('tamanoGas', '==' , 'Medianos').orderBy('fechaGas', 'desc').limit(10));
    this.ventasGas = this.GasCollection.valueChanges();
    this.data = [];
    const userDoc = this.GasCollection;
    userDoc.get().subscribe((querySnapshot) => {
  ...
  async loadResultsLimitGrandes() {
    ...
    // tslint:disable-next-line: triple-equals
    // tslint:disable-next-line: max-line-length
    this.GasCollection = this.db.collection('ventasGas', ref => ref.where('tamanoGas', '==' , 'Grandes').orderBy('fechaGas', 'desc').limit(10));
    this.ventasGas = this.GasCollection.valueChanges();
    this.data = [];
    const userDoc = this.GasCollection;
    userDoc.get().subscribe((querySnapshot) => {
  ...
  generarPDF() {
    ...
    this.data.forEach(user => {
     data.push([user.nombreGas, user.apellidoGas, user.cedulaGas, user.cantidadGas, user.tamanoGas]);
    ...
    const docDefinition = {
      ...
      },
      {
        ...
        alignment: 'center',
        style: 'subheader',
        text: 'Registro de las Ventas de Gas Realizadas '
      },
      {
        columns: [
          { width: '*', text: ''},
  ...
  async openPDF() {
    ...
    if (this.platform.is('cordova')) {
      ...
      this.pdf.getBuffer((buffer) => {
        ...
        const fileName = 'VentasGas.pdf';
        const saveDir = this.file.externalApplicationStorageDirectory;
        this.file.writeFile(saveDir, fileName , binaryArray, { replace: true })
